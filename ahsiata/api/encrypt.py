"""Encryption helpers and request signing for the main backend.

Wrappers around `ahsiata.core.crypto` plus:
- Device fingerprint (ax.fp) generation/loading
- Encrypted-field padding helpers (used by payment flows)
- Timestamp formatters (Java-style, GMT+7)
- `encryptsign_xdata` / `decrypt_xdata` for the main API request body
"""
...
from ahsiata.core.crypto import (
    decrypt_xdata as _decrypt_xdata,
...
def _build_fingerprint_plain(dev: DeviceInfo) -> str:
    return (
...
def ax_fingerprint(dev: DeviceInfo, secret_key_32hex_ascii: str) -> str:
    ...
    iv = b"\x00" * 16
    pt = _build_fingerprint_plain(dev).encode("utf-8")
    ct = AES.new(key, AES.MODE_CBC, iv).encrypt(pad(pt, 16))
    return base64.b64encode(ct).decode("ascii")


def load_ax_fp() -> str:
    """Load the device fingerprint from `ax.fp`; generate+persist if missing."""
    fp_path = "ax.fp"
    ...
    dev = DeviceInfo(
        ...
        font_scale=1.0,
        android_release="13",
        msisdn=CONFIG.device_fake_msisdn,
    )

    new_fp = ax_fingerprint(dev, CONFIG.ax_fp_key)
...
def ax_device_id() -> str:
    return hashlib.md5(load_ax_fp().encode("utf-8")).hexdigest()


# -- Encrypted-field padding --------------------------------------------------

def _random_iv_hex16() -> str:
    return os.urandom(8).hex()


def _b64(data: bytes, urlsafe: bool) -> str:
...
def encrypt_circle_msisdn(api_key: str, msisdn: str) -> str:
    return _encrypt_circle_msisdn(msisdn)


def decrypt_circle_msisdn(api_key: str, encrypted_msisdn_b64: str) -> str:
    return _decrypt_circle_msisdn(encrypted_msisdn_b64)
